...
from summarizer import gensim_summarize, spacy_summarize, nltk_summarize, sumy_lsa_summarize, sumy_luhn_summarize, \
    sumy_text_rank_summarize, sumy_lex_rank_summarize
...
def create_app():
    ...
    def transcript_fetched_query():
        ...
        if video_id and percent and choice:
            ...
            choice_list = ["gensim-sum", "spacy-sum", "nltk-sum", "sumy-lsa-sum", "sumy-luhn-sum", "sumy-text-rank-sum", "sumy-lex-rank-sum"]
                ...
                try:
                    ...
                    if select_length > 0:
                        ...
                        if num_sent_text > 1:
                            ...
                            output = [(summary[i:i+n]) for i in range(0, len(summary), n)]
                            hindi_summary = ""
                            # Printing outputs
                            # print(output)
                            for text in output:
                                hindi_summary= hindi_summary + " " + translater.translate(text, dest="hi").text
